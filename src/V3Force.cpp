...
public:
    ...
    static AstNodeExpr* createForceReadExpression(
        ...
        for (const auto& force : forces) {
            ...
            AstNodeExpr* const rhsClonep = force.second.m_rhsExprp->cloneTreePure(false);
...
public:
    ...
    void addForceAssignment(AstVar* varp, AstVarScope* vscp, AstNodeExpr* rhsExprp,
        ...
        V3Number zero{enVscp, enVscp->width()};
        zero.setAllBits0();
        ...
        ForceInfo forceInfo{rhsExprp, enVscp};
        m_forceInfo[varp].emplace(forceStmtp, forceInfo);
    }
...
class ForceDiscoveryVisitor final : public VNVisitorConst {
    ...
    void visit(AstAssignForce* nodep) override {
        ...
        m_state.addForceAssignment(lhsVarRefp->varp(), lhsVarRefp->varScopep(),
                                   nodep->rhsp()->cloneTreePure(false), nodep);
...
public:
    ...
    explicit ForceDiscoveryVisitor(AstNetlist* nodep, ForceState& state)
        : m_state{state} {
        iterateConstNull(nodep->modulesp());
    }
};
...
class ForceConvertVisitor final : public VNVisitor {
    ...
    void visit(AstRelease* nodep) override {
        ...
        for (const auto& force : forces) {
            ...
            V3Number zero{lhsp, ForceState::isRangedDType(releasedVarp) ? lhsp->width() : 1};
            zero.setAllBits0();
            AstNodeExpr* disableExprp = new AstAnd{flp, copyForDisablep, new AstConst{flp, zero}};
